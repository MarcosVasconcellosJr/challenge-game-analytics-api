...
import { LogLineParser, EventRowType } from './log-parser'
import { parse } from 'date-fns'

describe('LogParser', () => {
  it('should parse a kill event correctly', () => {
    const line = '12/02/2024 14:23:45 - Player1(TeamA) killed Player2(TeamB) using AK47'
    const result = new LogLineParser().parse(line)

    expect(result).toEqual({
      type: EventRowType.killEvent,
      timestamp: parse('12/02/2024 14:23:45', 'dd/MM/yyyy HH:mm:ss', new Date()),
      killer: 'Player1',
      killerTeam: 'TeamA',
      victim: 'Player2',
      victimTeam: 'TeamB',
